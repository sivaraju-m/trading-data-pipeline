...
def test_clean_ohlcv_data():
    ...
    assert not cleaned_df['volume'].isna().any(), "NaN values in 'volume' should be filled"
    
    # Check high/low corrections
    assert cleaned_df.loc[3, 'high'] >= cleaned_df.loc[3, 'low'], "High should be >= low"
    assert cleaned_df.loc[4, 'high'] >= cleaned_df.loc[4, 'open'], "High should be >= open"
    assert cleaned_df.loc[4, 'high'] >= cleaned_df.loc[4, 'close'], "High should be >= close"
