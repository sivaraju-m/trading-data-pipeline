...
class BigQueryManager:
        ...
        try:
            ...
            if not result.empty:
                ...
                return {
                    ...
                    "null_ohlc_percentage": (
                        (symbols_with_nulls / total_symbols * 100)
                        if total_symbols > 0
                        else 0
                    ),
                    "zero_volume_percentage": (
                        (symbols_with_zero_volume / total_symbols * 100)
                        if total_symbols > 0
                        else 0
                    ),
                    "quality_score": (
                        max(
                            0,
                            100
                            - (symbols_with_nulls / total_symbols * 50)
                            - (symbols_with_zero_volume / total_symbols * 30),
                        )
                        if total_symbols > 0
                        else 0
                    ),
    ...
    def run_data_quality_checks(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self.client.query(query).to_dataframe()
            symbols_updated = (
                result.iloc[0]["symbols_updated_yesterday"] if not result.empty else 0
            )
            ...
            if not dup_result.empty:
                ...
                issues.append(
                    f"{duplicate_count} duplicate records found in last 7 days"
                )
    ...
    def delete_symbol_data(self, symbol: str, date: str = None) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"🗑️ Deleted data for {symbol}" + (f" on {date}" if date else "")
            )
            return True
