...
class DailyDataScheduler:
    ...
    def load_config(self):
        ...
        if config_path.exists():
            ...
            self.config = {
                ...
                "data_sources": {"primary": "kiteconnect", "fallback": "yfinance"},
                "universe": {
                    "nifty50": True,
                    "nifty_next50": True,
                    "custom_symbols": [],
    ...
    def run_daily_data_pull(self):
        ...
        try:
            ...
            self.logger.info(
                f"📊 Results: {successful_updates} successful, {failed_updates} failed"
            )

            # Send completion notification
            self.send_completion_notification(
                successful_updates, failed_updates, duration
            )
    ...
    def pull_symbol_data(self, symbol):
        ...
        try:
            ...
            if data is None or data.empty:
                self.logger.warning(
                    f"⚠️ Primary source failed for {symbol}, trying fallback"
                )
    ...
    def validate_data_quality(self):
        ...
        try:
            ...
            if quality_report["issues"]:
                self.logger.warning(
                    f"⚠️ Data quality issues found: {quality_report['issues']}"
                )
    ...
    def generate_weekly_report(self):
        ...
        try:
            ...
            report_file = (
                f"reports/weekly_report_{datetime.now().strftime('%Y%m%d')}.json"
            )
    ...
    def send_completion_notification(self, successful, failed, duration):
        """Send completion notification"""
        self.logger.info(
            f"📧 Daily data pull completed - Success: {successful}, Failed: {failed}, Duration: {duration}"
        )
