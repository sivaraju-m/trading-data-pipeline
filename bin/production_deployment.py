...
class ProductionDeployment:
    ...
    async def setup_cloud_scheduler(self) -> Dict[str, Any]:
        ...
        try:
            response = self.scheduler_client.create_job(parent=parent, job=job)
    ...
    async def validate_historical_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if actual_latest < expected_latest:
                logger.warning(
                    f"Data gap detected. Latest date: {actual_latest}, Expected: {expected_latest}"
                )
...
async def main():
    ...
    logger.info(
        f"📅 Data range: {validation_result['earliest_date']} to {validation_result['latest_date']}"
    )
